...
bool th_create = false;
int bg_alloc = 0;
int shared_v = 0;

arena_t* global_arena;
arena_chunk_t* next_chunk;
#endif
...
/******************************************************************************/
#ifdef WKLEE
void *arena_chunk_alloc_background_thread(void* arg) {
	...
	while(1){
		...
		if(likely(global_arena != NULL)){
			arena = global_arena;
			malloc_mutex_lock(&arena->lock);
			next_chunk = arena_chunk_alloc(arena);
			//chunk = arena_chunk_alloc(arena);
			malloc_mutex_unlock(&arena->lock);
		}
		bg_alloc = 1;
			//printf("$$$$$$$$$$$Background alloc complete.$$$$$$$$$$$$$$$$\n");
	}
}
...
void threadInit() {
    ...
    if (pthread_create(&thread, &attr, arena_chunk_alloc_background_thread, (void*) NULL) != 0) {
    ...
    size_t flag_dirty, size_t flag_decommitted, size_t need_pages)
{
...
#ifdef WKLEE
	if(likely(th_create)){
		if ((arena->ind != 0) && (bg_alloc) && ((next_chunk == NULL) || (chunk == next_chunk)) && (total_pages < (chunk_npages-map_bias) / 2)) {
		//if ((arena->ind != 0) && (bg_alloc) && (chunk == next_chunk) && (total_pages < (chunk_npages-map_bias) / 2)) {
			global_arena = arena;
			pthread_mutex_lock(&mutex_a);
			shared_v = 1;
	...
	//printf("arena[%u]\t total pages: %zu\t", arena->ind, total_pages, rem_pages);
	//printf("current chunk: %p\t next chunk: %p\n", chunk, next_chunk);
	//printf("current chunk: %p\n", chunk);
#endif
...
#if 0
void *arenaStatusCheckThread(void* arg) {
	arena_t *arena;
	arena_chunk_t *chunk;

	unsigned narenas;

	size_t PG_SIZE, CH_SIZE;
	size_t pused, pactive, pdirty, mapped;
	size_t remain;

	size_t epoch;
	size_t sz;
	size_t allocated, active, metadata, resident, s_mapped, diff;
	...
	while(1){
	    ...
	    if (arena < 0) {
            ...
            jemk_mallctl("stats.mapped", &s_mapped, &sz, NULL, 0);

			diff = active - allocated;

            printf("resident: %zu allocated: %zu active: %zu mapped: %zu\n", resident, allocated, active, s_mapped);
#endif
		...
		if (diff >= threshold) {
			...
			if (chunk == NULL) {
				printf("arena_chunk_alloc returned NULL\n");
			}else {
